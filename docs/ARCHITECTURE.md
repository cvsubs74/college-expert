...
|---|---|---|---|
| 2026-07-02 | #310 | Bundled add-college analysis + agentic save route (extends epic #280). **Backend (profile_manager_v2):** new `POST /add-college-analysis {user_email, university_id, force?}` (`add_college_analysis.py`) — ONE credit gate (mirrors `fit_billing`: check → 402 / 503 on the #298 `credits_read_failed` marker → generate → deduct once AFTER success, never on failure, never on a fallback fit) around a single billed unit that produces BOTH artifacts: (a) the fit via `calculate_fit_for_college` (resolution-order major) + save-with-archive, then (b) the major-chances ranking via `major_llm.run_ranking_generation` (the extracted UNBILLED core of `run_rank_college_majors`) + save-with-archive. The fit is the primary artifact — its failure/save-failure is a 500, unbilled; a KB-majors miss (or ranking hiccup) degrades `major_chances` to null + note (+kb_gap on a real miss), never a second charge. New shared `analysis_schema.py` (FIT_SCHEMA + MAJOR_CHANCES_SCHEMA: fields/types/enums/required + human-readable `trust_rules`; `validate_against(kind,payload)`) — the SINGLE source for both the describe surface and agent-write validation. New agent-only `GET /get-analysis-schema?kind=fit|major_chances` (NOT wired into frontend api.js). New FREE trust-enforced agent writes (`agent_writes.py`): `POST /save-external-fit` re-applies the EXTRACTED `fit_computation.post_process_fit` (selectivity floor/ceiling, match% band clamp, factor bounds, don't-submit-when-no-scores — now shared by the in-app LLM path unchanged) and sources acceptance_rate + KB provenance from the KB (a fabricated category is floored); `POST /save-external-major-chances` runs the agent's `{majors:[{name,tier,rationale}]}` through the shared `major_llm.assemble_and_save_ranking` → `normalize_college_major_ranking` (entry_path/entry_risk re-derived from KB, fabricated %/GPA stripped, off-catalog names dropped, capped_door door-lock, tiers coerced); both stamp `source`(claude/chatgpt)/`basis:inference`, archive prior, 0 credits; a school with no KB majors → 400. **Connector:** `get_analysis_schema` (readOnly) + free `save_fit_analysis`/`save_major_chances` (write rate-guard, `_client_attribution` source, honest field-error relay); server instructions gain the credit-saving recipe. **Frontend:** add-to-Launchpad + the card's regenerate call `addCollegeAnalysis` (1 credit, server-billed, both artifacts; keeps the hasCredits pre-check + CreditsUpgradeModal, no client deduct); UniversityCard surfaces a free "Update via ChatGPT/Claude" `AgentChatHandoff` (prompt from `mcpClients.agentUpdateAnalysisPrompt`); the schema is never rendered in the UI. Granular `compute-single-fit`/`rank-college-majors` kept for single-artifact in-app regen. | Owner decision (2026-07-02): adding a college should always produce both the fit AND the major chances for one credit, and regeneration should have a FREE agent path where ChatGPT/Claude compute + save the analyses themselves — which only works if every agent write is re-validated and trust-enforced server-side (reusing the exact in-app normalizers) so an agent can't persist fabricated data (design: `docs/design/DESIGN-major-strategy.md` §Bundle + agent-write trust) |
| 2026-07-02 | #306 | Major Map grounded in the global catalog: `generate-major-map` fetches `?action=majors-catalog` (top-250), injects it as the candidate universe (LLM draws only from real, offered majors), and `ground_clusters_in_catalog` matches each suggestion (display-name via the single major_match normalizer — sidesteps the KB-vs-PM key drift) → canonical name + `offered_count` + `in_catalog:true`, or `in_catalog:false` for anything not really offered (flagged, not silently presented). Catalog-fetch failure degrades to unconstrained generation + a data note. major_match `_SUFFIX_RE` gains program/track/concentration for catalog parity. | The school-agnostic Major Map suggested free-form LLM names with no guarantee any college offered them; it now only surfaces real majors (completes #303's deferred AC) |
| 2026-07-02 | #302 | Per-college "Major Chances" (extends epic #280): Major Map promoted from a card buried in Profile to its own sidebar route `/major-map` (new `pages/MajorMap.jsx` renders the unchanged `MajorMapCard` off a lightweight get-profile fetch; removed from Profile). New billed endpoint `POST /rank-college-majors` (profile_manager_v2, 1 credit, reason `major_chances`, via the same `generation_billing.run_billed_generation`) — a SIBLING of generate-major-strategy on `major_llm.py`: deterministic `?action=majors` fetch FIRST so a school absent from the KB (or with zero majors) returns 200 `{ranking:null, gaps}` UNBILLED and increments `kb_gaps` (never-charge-on-miss); the LLM scans the FULL labeled catalog (`build_full_catalog_rows` → the existing `build_labeled_extract`) and SELECTS the fit majors (incl. strategic/adjacent doors the student didn't ask about), ranking them into likelihood TIERS `strong/possible/reach/long_shot` with a rationale each — NEVER a fabricated percentage (we lack verified per-major rates). Post-process reuses the trust machinery verbatim: names bound to real catalog majors (invented ones dropped), entry_path/entry_risk taken from the KB row (not the LLM), `validate_numeric_claims` strips any %/GPA absent from the extract out of every rationale, `capped_door` majors get a deterministic door-lock caveat appended, and a reported rate rides along ONLY where the KB has one (labeled "reported (unverified)"). An empty fit-selection (LLM finds no fit — rare) is a REAL generation: 200 + empty tiers + note, and IS billed (distinct from the unbilled KB miss). Persisted `users/{email}/college_major_chances/{id}` (basis `inference`) with archive to `history/{kb_data_year}`; free read `get-college-major-chances` (kb_data_year staleness). Connector: `rank_college_majors` (recompute-class rate guard, shared 402 helper, honest miss relay) + `get_college_major_chances` (read). Frontend: Launchpad `UniversityCard` gains a "Major Chances" action (`onMajorChances`) opening `MajorChancesView` in place (same pattern as FitAnalysisPage) — tiered sections, entry-path/entry-risk badges, "Stratia's read — inference" chip per major, hedged reported-rate line, credit-gated CTA, KB-miss/stale/verification states; api.js `rankCollegeMajors`/`getCollegeMajorChances` | Phase-2 gave per-school synthesis for the majors a student already named; students still had no view of which majors a school ACTUALLY OFFERS fit them, ranked — and any likelihood surface needed the same anti-fabrication discipline (tiers not invented odds, never charge for "we don't know") (design: `docs/design/DESIGN-major-strategy.md`) |
| 2026-07-02 | #303 | Global major catalog: new KB doc `major_catalog/current` (union of every major across all profiles, normalized-keyed, idempotent per-school `university_ids` sets; `major_catalog.py` pure logic + firestore_db read/write). Maintained best-effort at ingest (`ingest_university` hook, guarded so a catalog failure never fails an ingest); full rebuild via `scripts/build_major_catalog.py`; read via `GET ?action=majors-catalog`; MCP `list_major_catalog`. | Grounds the school-agnostic Major Map in real, offered majors instead of free-form LLM names (consumption change follow-up on #302); reusable major universe (~369 majors / 192 schools) |
| 2026-07-02 | #223 | Backends verify caller identity: new `request_auth.py` (identical copy per service) verifies Firebase ID tokens (users) and audience-bound Google OIDC service tokens (connector/counselor, allowlisted via TRUSTED_SERVICE_EMAILS + SELF_AUDIENCES); profile-manager-v2 and counselor-agent gate every per-user route (health/clear-test-data exempt), KB gates writes (reads stay public; ingest CLI uses the new kb-write-token secret). Frontend attaches Firebase tokens via an axios interceptor (both instances); counselor attaches service tokens on its 11 PM call sites (svc_auth.py); connector attaches per-audience cached tokens in _get/_post. Rollout via AUTH_MODE env: off/log/enforce (deploy default log; flip with AUTH_MODE=enforce ./deploy.sh <target>) | The live backends trusted a raw caller-supplied X-User-Email — anyone could read/mutate any student's data or drain credits (IDOR); the verified identity now scopes access (design: docs/design/DESIGN-backend-auth.md) |
| 2026-07-01 | #284 | Major Strategy phase 2 (epic #280): profile_manager_v2 gains two billed LLM endpoints on new `major_llm.py` + shared `generation_billing.run_billed_generation` (mirrors fit_billing's check → 402 → generate → deduct-once-after-success invariants). `POST /generate-major-map` (1 credit, reason `major_map`): readiness 422 `profile_incomplete` (grade + ≥2 of courses/ECs/gpa_*, unbilled) → cache-unless-force on a `profile_fingerprint` (sha1 + per-part hashes of intended_majors/course names/EC names/grade) → grade-aware clusters citing the student's own record; persisted `users/{email}/major_map/current` with archive to `major_map_history/{generated_at}`. `POST /generate-major-strategy` (1 credit, reason `major_strategy`): deterministic extraction FIRST — server-side `?action=majors` fetch, exact/strong `match_major` filter (+same-college backups) — so a KB miss returns 200 `{strategy:null, gaps}` UNBILLED and increments root `kb_gaps/{university_id}` (demand queue for #193); synthesis prompts over ONLY the `[VERIFIED]/[REPORTED]/[OPINION]/[MISSING]`-tagged extract, then a deterministic numeric-claim validator strips any %/GPA token absent from the extract into `data_notes`, and a deterministic capped_door door-lock warning appends to `primary_call` when the LLM omitted it; persisted `users/{email}/major_strategies/{id}` with archive to `history/{kb_data_year}`. Free reads `get-major-map` (fingerprint staleness + reasons) and `get-major-strategy` (kb_data_year staleness). `set-major-choice` now stamps `major_choice.door_flags {entry_path, entry_risk}` from a best-effort `action=majors&q=` fetch (never blocks). Connector: `get_major_map`/`get_major_strategy` (free) + `generate_major_map`/`generate_major_strategy` (recompute-class rate guard, shared 402 helper, honest miss relay). Frontend: Profile `MajorMapCard` (readiness-aware CTA, evidence chips, relation badges, set-intended-majors action, stale banner — never auto-regenerates), Majors-tab `MajorStrategySynthesis` ("Stratia's read" sections incl. ESSAY IMPLICATION + verify-yourself checklist + stale chip), Launchpad `capped_door` callout on UniversityCard (deterministic, zero LLM) | Phase 1 shipped honest facts; students still had no discovery surface (only the one major they know) and no per-school synthesis — and any LLM synthesis over thin KB data needed hard anti-fabrication mechanics (never charge for "we don't know", never emit numbers the KB doesn't hold) (design: `docs/design/DESIGN-major-strategy.md` §Phase 2) |
| 2026-07-01 | #285 | `compute-single-fit` bills server-side again (restores the legacy ES contract in the active Firestore handler): new `fit_billing.run_compute_single_fit` sequences cache-unless-force → 402 `insufficient_credits` (+`credits_remaining`) → compute → `deduct_credit(1,'fit_analysis')` exactly once AFTER compute+save; absent `force_recompute` defaults to **true** (compat: QA agent + pre-#285 MCP clients never send it), explicit `false` returns the cached fit free (`from_cache:true`). Frontend drops all client-side `deductCredit` calls for fit computes (Launchpad add-school/recompute-with-major, Explorer add-to-list) and relies on the 402 → CreditsUpgradeModal path; KB-refresh "Update fit" flows now send `force_recompute:true` (explicit false would return the stale cache). Connector `recompute_fit` sends `force_recompute:true` and surfaces 402 as a clear budget error (`_post` now attaches `status_code`/`body` to `StratiaError`) | The active handler had no cache/credit/deduct logic — the app deducted client-side (double-charge risk once the server billed) and MCP `recompute_fit` claimed "1 credit" but was free |
| 2026-07-01 | #286 | Year-history consumers (follow-up to #279): `university_chat` system prompt gains a fault-isolated `YEARLY ADMISSIONS HISTORY` block — built once per request from `get_university_history` inside try/except (any failure just drops the block), injected only when snapshots + reported_trends total ≥2 rows, null-stripped compact JSON, labeled (snapshots authoritative/cycle-year vs trends UNVERIFIED/entering-class) with explicit snapshot precedence + never-merge instruction; first test coverage for `university_chat`. Frontend: new `api.getUniversityHistory` (`?action=history`, v2 fallback URL) + `AdmissionsHistoryChart` replaces AdmissionsTab's inline longitudinal_trends bar chart — two labeled groups (KB snapshots with `estimated year` marker; school-reported muted/dashed with different-axis caption), selectivity indicator computed within one series only (reported ≥2, else snapshots ≥2, never across), single-row "history builds up" state, legacy trends rendering kept as the fetch-failure fallback | The verified collector writes only ONE `longitudinal_trends` row, so the old chart degrades to a single bar as re-collection proceeds; chat answered trend questions from the unverified profile-baked rows because the prompt injected only the current profile |
| 2026-07-01 | #281/#282 | Major Strategy phase 1 (epic #280): KB gains `?action=majors` — deterministic trust-labeled per-major extract (new `major_facts.py`: entry_path classifier w/ honest 'unclear' bucket, structural `entry_risk` incl. capped_door, basis labels kb_verified/kb_reported/opinion, richness_tier). profile_manager_v2 gains `save-onboarding-profile` (the route OnboardingModal always posted to — onboarding majors/preferences were silently lost), `set-intended-majors` (≤5 ranked + `intended_major` mirror), `set-major-choice` (KB-name match ladder via new `major_match.py`; exact/strong auto-bind, fuzzy stored matched:false; `major_choice` + `selected_major` mirror on the list item), `compute-single-fit` resolution order (request → major_choice → selected_major → profile) stamped as `intended_major_used`/`intended_major_source`; `get_college_list` enrichment returns `selected_major`/`major_choice` (round-trip fix). Connector: `get_university_majors`, `set_intended_majors`, `set_major_choice`, `recompute_fit(major=)` + major-session instructions w/ anti-backdoor + school-type guardrails | Students apply to overcompetitive majors blind; profile had a single intended_major; onboarding majors never persisted; Launchpad selected_major never round-tripped (design: `docs/design/DESIGN-major-strategy.md`) |
