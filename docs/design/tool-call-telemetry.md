...
> **Chosen direction (product steer, supersedes the telemetry approach below):**
> No BigQuery, no per-tool instrumentation. Instead, capture the workflow
> **at save time from the agent itself** — Claude knows the steps it ran, so
> `save_research` accepts the ordered `workflow` (and the original ask). The
> workflow rides on the research doc in Firestore. That powers (1) a **"Repeat
> this workflow" widget** on each saved research and (2) a **"Popular Workflows"
> dashboard** aggregating workflow signatures across researches. See
> "Revised direction" at the bottom. The telemetry/BigQuery sections below are
> kept as the rejected-alternative analysis.
...
## Revised direction (chosen): agent-reported workflows on the research

No instrumentation, no BigQuery. The agent self-reports the workflow when it
saves a research.

### Capture (no instrumentation)
Extend `save_research` with two optional fields:
- `source_prompt` — the user's originating request, in the user's voice
  ("Compare Duke and UCSD for CS and tell me which is more realistic").
- `workflow` — the ordered steps the agent ran, each a short
  `{tool, label}` (e.g. `{tool:"get_fit_analysis", label:"Pulled Duke & UCSD fit"}`).

Stored on `users/{email}/research/{id}.workflow` (+ `source_prompt`). Claude
reliably knows its own steps, so this needs zero server-side correlation.

### Surface 1 — "Repeat this workflow" widget (per research)
On `ResearchCard`, when a note has a `workflow`/`source_prompt`, show a compact
step list and a **"Run again in Claude / ChatGPT"** action: a generated prompt
that re-issues the workflow (reusing the `/connect` `askLinks()` deep-link
pattern) + Copy. The app does **not** execute MCP tools itself — "repeat" means
re-running it in the connected agent.

### Surface 2 — "Popular Workflows" dashboard
Aggregate saved-research workflows in Firestore by a normalized **signature**
(the ordered tool-name sequence, and/or the research `kind`). Show the most
common workflows as launchable templates (same deep-link "Run" action). At our
volume an in-app aggregation (read research across users, or maintain small
per-signature counters on save) is sufficient — no warehouse.

### Privacy
`source_prompt`/`workflow` labels are agent-authored summaries; keep them free of
raw PII (the agent is instructed to summarize steps, not dump payloads). The
popular-workflows aggregation uses tool-name signatures + counts, not user data.

### Phasing
1. Capture (`save_research` fields + storage) — foundation.
2. "Repeat this workflow" widget on `ResearchCard`.
3. "Popular Workflows" dashboard/section.
