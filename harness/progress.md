...
## 2026-07-01 12:22 — shipped chat fault tolerance + agentic escape hatch (PR #276)
...
## 2026-07-01 19:55 — #284 Major Strategy phase 2 implemented, PR #300 open (implementer)
- profile_manager_v2: new major_llm.py + generation_billing.py (fit_billing-grade check→402→generate→deduct-once). generate-major-map (readiness 422 unbilled, fingerprint cache-unless-force, archive to major_map_history) + generate-major-strategy (extraction BEFORE the credit gate → KB miss = 200 {strategy:null,gaps} unbilled + kb_gaps/{id} increment; labeled [VERIFIED]/[REPORTED]/[OPINION]/[MISSING] extract; deterministic numeric-claim validator strips unextracted %/GPA into data_notes; deterministic capped_door door-lock append). Free get-major-map/get-major-strategy with staleness. set-major-choice stamps door_flags via action=majors&q= (best-effort).
- Connector: 4 tools (2 free reads, 2 recompute-guarded generates w/ shared 402 helper + honest miss relay); instructions extended.
- Frontend: Profile MajorMapCard (evidence chips, relation badges, set-intended-majors, stale banner never auto-regen), Majors-tab MajorStrategySynthesis ("Stratia's read" + ESSAY IMPLICATION + verify checklist), Launchpad capped_door callout (zero LLM).
- Tests: pytest 1192 passed (70 new, LLM monkeypatched); connector 103 passed; vitest 442 passed (25 new); build + verify.sh green.
- Decision: "matched" for the miss rule = exact/strong binds only (fuzzy → RELATED/BACKUP rows, never headline facts). KbRefreshReviewModal-style change card (issue summary, not an AC) deferred to the KB-refresh flow.

## 2026-07-02 — #310 bundled add-college analysis + agentic save route, PR #314 (+ review hardening)
- Owner ask: adding a college to the Launchpad should cost 1 credit to generate BOTH the fit and the major-chances; regeneration also 1 credit UNLESS the student uses their own ChatGPT/Claude to compute it and saves it back (free); the agent must be told the result SHAPE without exposing that schema to the human user.
- Decisions (AskUserQuestion): (1) "regeneration only" — the add always costs 1 credit; agent route saves credits only on regenerate. (2) "agent-only schema tool" — `get_analysis_schema` MCP tool feeds the shape from `analysis_schema.py` (single source for both the describe surface AND validate_against); NEVER wired into frontend/api.js.
- Server: `POST /add-college-analysis` (one credit around the fit+chances bundle; KB-miss → chances:null + kb_gap, fit still computed). FREE agent-write endpoints `/save-external-fit` + `/save-external-major-chances` (`agent_writes.py`) re-apply the exact in-app trust machinery server-side. Connector: `get_analysis_schema`, `save_fit_analysis`, `save_major_chances` (source connector-attributed via `_client_attribution` — agent can't spoof source='stratia').
- Adversarial security review (subagents) found + I fixed 3 must-fix on the save-fit surface: extra-field passthrough (now a content allow-list — no logo_url render-sink injection), floor-dodge on KB-miss (now 400 + acceptance_rate always from KB), unbounded ranking (≤200 majors). Verified solid: fit floor with KB present, chances trust machinery, exact-once billing, first-party provenance un-spoofable, schema not in frontend, no stored XSS.
- Tests: profile_manager_v2 379 passed (incl. new allow-list/KB-miss/oversized-list regressions), connector 122 passed, add-college + schema suites green. verify + college-expert-pr CI on the PR.
- ⚠️ STANDING GATE: these endpoints (credit burn + cross-user writes) are only safe once [[backend-auth]] AUTH_MODE=enforce on profile_manager_v2 — still in LOG mode (IDOR-open). Enforce flip remains the pending manual step.
