...
def essay_chat(
    ...
    try:
        ...
        try:
            ...
            fit_response = es_client.search(index="fit_analyses", body=fit_query)
            if fit_response["hits"]["hits"]:
                fit_analysis = fit_response["hits"]["hits"][0]["_source"]
        except Exception as e:
            logger.warning(f"[ESSAY_CHAT] Fit analysis not found: {e}")
...
def save_essay_draft(
    ...
    Args:
        ...
        version: Version number (0 = main, 1+ = alternate versions)
        version_name: Optional name for the version (e.g., "Community angle")
    ...
    try:
        ...
        # Create unique document ID including version
        doc_id = f"{user_email}_{university_id}_{prompt_index}_v{version}"
        ...
        logger.info(f"[ESSAY_COPILOT] Saved draft: {doc_id}, {doc['word_count']} words, version={version}")
