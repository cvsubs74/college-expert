...
# Guard: deploys must run from a clean main checkout
# ---------------------------------------------------
# Background: this repo previously deployed via `--source=.` from whichever
# worktree happened to be active, including feature branches mid-development.
# The result: prod ran code that was never on main, and main became a lying
# snapshot of what's deployed. Investigations had to dig through worktree
# state instead of `git log main`.
#
# The guard refuses to deploy unless:
#   1. HEAD is on the `main` branch (not a feature branch, not detached)
#   2. The working tree is clean (no uncommitted modifications to tracked files)
#
# Untracked files (e.g., the .claude/ session dir) are tolerated.
#
...
if [ "${DEPLOY_ALLOW_DIRTY:-0}" != "1" ]; then
    ...
    if [ "$CURRENT_BRANCH" != "main" ]; then
        echo -e "${RED}╔════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${RED}║  Refusing to deploy: not on 'main'                         ║${NC}"
        echo -e "${RED}╚════════════════════════════════════════════════════════════╝${NC}"
        echo -e "${RED}Current branch: ${CURRENT_BRANCH:-<detached HEAD>}${NC}"
        echo -e "${YELLOW}Deploys must run from a clean main checkout after PR merge.${NC}"
        echo -e "${YELLOW}  • Open a PR for your changes${NC}"
        echo -e "${YELLOW}  • Merge to main once CI passes${NC}"
        echo -e "${YELLOW}  • git checkout main && git pull && ./deploy.sh ${DEPLOY_TARGET}${NC}"
        echo ""
        echo -e "${YELLOW}Emergency override (NOT recommended):${NC}"
        echo -e "${YELLOW}  DEPLOY_ALLOW_DIRTY=1 ./deploy.sh ${DEPLOY_TARGET}${NC}"
        exit 1
    fi
    # `git status --porcelain` only lists tracked-file changes by default;
    # `-uno` further suppresses untracked entirely so the .claude/ session dir
    # and other working-tree-only artifacts don't trigger the guard.
    if [ -n "$(git status --porcelain -uno 2>/dev/null)" ]; then
        echo -e "${RED}╔════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${RED}║  Refusing to deploy: working tree has uncommitted changes  ║${NC}"
        echo -e "${RED}╚════════════════════════════════════════════════════════════╝${NC}"
        echo -e "${YELLOW}Tracked files have local modifications:${NC}"
        git status --short -uno | head -20
        echo ""
        echo -e "${YELLOW}Commit your changes via a PR before deploying.${NC}"
        echo -e "${YELLOW}Emergency override: DEPLOY_ALLOW_DIRTY=1 ./deploy.sh ${DEPLOY_TARGET}${NC}"
        exit 1
    fi
    # Soft warning: the local main may have drifted from origin/main (e.g.,
    # someone merged a PR and the local checkout hasn't pulled). Don't block
    # — `git pull` is part of the documented workflow and warrants a heads-up,
    # not a refusal.
    git fetch origin main --quiet 2>/dev/null || true
    LOCAL_HEAD=$(git rev-parse HEAD 2>/dev/null || echo "")
    REMOTE_HEAD=$(git rev-parse origin/main 2>/dev/null || echo "")
    if [ -n "$LOCAL_HEAD" ] && [ -n "$REMOTE_HEAD" ] && [ "$LOCAL_HEAD" != "$REMOTE_HEAD" ]; then
        echo -e "${YELLOW}⚠ Warning: local main (${LOCAL_HEAD:0:8}) differs from origin/main (${REMOTE_HEAD:0:8})${NC}"
        echo -e "${YELLOW}  Run 'git pull' if you want the latest before deploying.${NC}"
        echo ""
    fi
    echo -e "${GREEN}✓ Deploying from clean main @ ${LOCAL_HEAD:0:8}${NC}"
else
    echo -e "${YELLOW}╔════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${YELLOW}║  ⚠ DEPLOY_ALLOW_DIRTY=1 — branch + cleanliness guard SKIPPED ║${NC}"
    echo -e "${YELLOW}╚════════════════════════════════════════════════════════════╝${NC}"
    echo -e "${YELLOW}Deploying from $(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo '<detached>') with whatever's in the working tree.${NC}"
    echo -e "${YELLOW}Follow up with a real PR ASAP so main matches prod.${NC}"
    echo ""
fi
