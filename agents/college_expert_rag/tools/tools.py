...
def list_documents() -> Dict[str, Any]:
    ...
    Returns:
        Dictionary with list of documents including their names and metadata
    """
    try:
        # Call the knowledge base manager cloud function
        url = f"{KNOWLEDGE_BASE_MANAGER_RAG_URL}/documents"
        
        headers = {
            "Content-Type": "application/json",
            "X-User-Email": "knowledge_base_user"
        }
        ...
        if result.get("success"):
            ...
            return {
                ...
                "message": f"Found {len(documents)} documents in the knowledge base"
            }
        else:
            return {
                "success": False,
        ...
        logger.error(f"Knowledge base list request failed: {e}")
        return {
            "success": False,
            "error": str(e),
...
def search_knowledge_base(
    ...
    try:
        ...
        if result.get("success"):
            # Get the response text from File Search
            ...
            for doc in documents:
                ...
                if snippets:
                    citations.append({
                        "source": doc.get("display_name", "Unknown"),
                        "text": snippets[0][:300] if snippets[0] else ""
                    })
            ...
            return {
                ...
                "query": query,
                "answer": response_text if response_text else f"Found {len(documents)} relevant documents for: {query}",
                "citations": citations,
